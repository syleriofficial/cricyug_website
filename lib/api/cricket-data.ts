...
import type { Match, Player, NewsArticle, Series, Team, SearchResult, PointsTableEntry } from "@/lib/types"
...
class CricketDataService {
  ...
  async getSeriesStandings(seriesId: string): Promise<PointsTableEntry[]> {
    const data = await this.request<unknown[]>("/series_points", { id: seriesId })
    return this.transformStandings(data)
  }
  ...
  private transformStandings(data: unknown[]): PointsTableEntry[] {
    if (!Array.isArray(data)) return []
    return data.map((item, index) => {
      const s = item as Record<string, unknown>
      const teamInfo = s.teamInfo as Record<string, unknown> || {}
      return {
        position: index + 1,
        team: {
          id: String(teamInfo.id || s.teamId || ""),
          name: String(teamInfo.name || s.teamName || ""),
          shortName: String(teamInfo.shortname || "").toUpperCase() || String(teamInfo.name || "").substring(0, 3).toUpperCase(),
          logo: String(teamInfo.img || ""),
        },
        played: Number(s.matchesPlayed || s.played || 0),
        won: Number(s.matchesWon || s.won || 0),
        lost: Number(s.matchesLost || s.lost || 0),
        tied: Number(s.matchesTied || s.tied || 0),
        noResult: Number(s.noResult || s.nr || 0),
        netRunRate: Number(s.nrr || 0),
        points: Number(s.points || s.pts || 0),
        recentForm: [],
      }
